...
// @resource     iconApi https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/api.svg
// @resource     iconUi https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/ui.svg
// @resource     iconData https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/data.svg
// @resource     iconPosTL https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/topleft.svg
// @resource     iconPosTR https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/topright.svg
// @resource     iconPosBL https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/botleft.svg
// @resource     iconPosBR https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/botright.svg
// @resource     iconAudit https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/audit.svg
// @resource     iconReset https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/reset.svg
// @resource     iconVault https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/vault.svg
// @resource     iconLotto https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/lotto.svg
// @resource     iconStop https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/stop.svg
// @resource     iconBuy https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/buy.svg
// @resource     iconCrown https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/crown.svg
// @resource     iconSlow https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/slow.svg
// @resource     iconError https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/error.svg
// @resource     iconCheck https://raw.githubusercontent.com/Photaz/GiftMAM/refs/heads/dev/assets/check.svg
...
(function() {
    ...
    style.textContent = `
        ...
        .mam-btn-run.stopping { background: rgba(239, 83, 80, 0.25); }
        .mam-btn-run.stopping:hover { background: #e53935; }
    ...
    const logIcon = (name) => `<img src="${icons[name]}" style="width: 14px; height: 14px; vertical-align: middle; margin-right: 4px; filter: drop-shadow(1px 1px 0px rgba(0,0,0,0.5));">`;
        ...
        <div class="mam-view" id="mam-view-about">
            <div class="mam-about-container" style="text-align: center; position: relative; height: 100%; box-sizing: border-box; display: flex; flex-direction: column; justify-content: space-between;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px; margin: 8px 0;">
                    ...
                    <div style="display: flex; justify-content: center; gap: 12px;">
                        <a href="https://www.myanonamouse.net/f/t/89069/p/1" target="_blank" style="color: #5EB9FF; text-decoration: none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">Forum Post</a>
                        <span style="color: var(--mam-border);">|</span>
    ...
    const StateManager = {
        ...
        updateBP(bp) {
            ...
            const uiBp = document.querySelector('.mam-stats span[title="Bonus Points"] span:not(.mam-emoji)');
            if (uiBp) {
                if (bp === null) {
                    uiBp.textContent = '...';
                } else {
                    uiBp.textContent = (bp >= 1000) ? Math.floor(bp / 1000) + 'K' : bp.toString();
                    uiBp.parentElement.title = 'Bonus Points: ' + bp.toLocaleString('en-US');
                }
            }
    ...
    const HistoryManager = {
        ...
        async auditQueue(candidates) {
            try {
                ...
                return candidates.filter(c => !this.giftedUserIds.has(c.id.toString()));
            } catch (error) {
                return candidates;
            }
        }
    ...
    const QueueManager = {
        ...
        async loadAndMerge(newTargets, isRefresh = false) {
            ...
            uniqueUsers = uniqueUsers.filter(u => !Database.has(u.id, u.name));
            this.users = await HistoryManager.auditQueue(uniqueUsers);
    ...
    const Engine = {
        ...
        stop() {
            ...
            StateManager.state.isRunning = false;
            Logger.log(`${logIcon('slow')} Stopping...`);
            this.resetUI();
        },
        ...
        async start() {
            ...
            if (btn) {
                ...
                btn.innerHTML = `<img src="${icons.stop}" style="width: 16px; height: 16px; vertical-align: middle;">`;
            }
            ...
            for (let i = 0; i < safeTargets.length; i++) {
                ...
                if (StateManager.state.currentBP !== null && StateManager.state.currentBP < minReserve) {
                    abortReason = `${logIcon('stop')} BP hit safety floor (${minReserve}). Stopping.`;
                    break;
                }
                ...
                try {
                    ...
                    if (!data.success) {
                        ...
                        if (errStr.includes("rate limit")) {
                            Logger.log(`${logIcon('slow')} API Limit hit. Soft pausing for 15s...`);
                            let slept = 0;
                            while (slept < 15000) {
                                if (!StateManager.state.isRunning) {
                                    abortReason = "stopped";
                            ...
                            // Quiet Adoption (Skip)
                            Logger.log(`${logIcon('error')} ${user.name}: ${data.error} (Adopted)`);
                    ...
                    // Success handling
                    Logger.log(`${logIcon('check')} ${user.name} (${giftAmount})`);
                    ...
                    Logger.log(`${logIcon('error')} ${user.name}: ${e.message}`);
                }
            }
    ...
    const DailiesManager = {
        init() {
            ...
            // Run a daily evaluation trigger for VIP status on init if allowed
            if (StateManager.state.config.renewVip) {
                setTimeout(() => Engine.triggerHeartbeat(), 5000);
            }
    ...
    // Event-based Console Testing Hooks (Bypasses Sandbox Isolation)
    window.addEventListener('mam-test-vault', () => {
        GM_setValue('mam_vault_next_reset', '0');
        GM_setValue('mam_lotto_next_check', (Date.now() + 86400000).toString());
        StateManager.init();
        DailiesManager.updateUI();
        console.log('[GiftMAM Test] Vault reminder forced.');
    });

    window.addEventListener('mam-test-lotto', () => {
        GM_setValue('mam_lotto_next_check', '0');
        GM_setValue('mam_vault_next_reset', (Date.now() + 86400000).toString());
        StateManager.init();
        DailiesManager.updateUI();
        console.log('[GiftMAM Test] Lotto reminder forced.');
    });

    window.addEventListener('mam-test-both', () => {
        GM_setValue('mam_vault_next_reset', '0');
        GM_setValue('mam_lotto_next_check', '0');
        StateManager.init();
        DailiesManager.updateUI();
        console.log('[GiftMAM Test] Both reminders forced.');
    });
    ...
    document.getElementById('btn-open-changelog').addEventListener('click', (e) => {
        ...
        fetch('https://api.github.com/repos/Photaz/GiftMAM/releases/latest')
            .then(res => {
